...
cache_aligned_unique_ptr<T> make_cache_aligned_unique(Args &&...args) {
  return cache_aligned_unique_ptr<T>(new (cache_aligned_allocate(sizeof(T)))
...
//! Execute data used on a task dispatcher side, reflects a current execution
//! state
...
struct task_accessor {
  ...
  static d1::task_group_context *&context(d1::task &t) {
    task_group_context **tgc =
        reinterpret_cast<task_group_context **>(&t.m_reserved[0]);
    return *tgc;
  }
...
/** The standard offsetof macro is not sufficient for TBB as it can be used for
    POD-types only. The constant 0x1000 (not nullptr) is necessary to appease
   GCC. **/
...
#define __TBB_get_object_ref(class_name, member_name, member_addr)             \
  (*reinterpret_cast<class_name *>((char *)member_addr -                       \
                                   __TBB_offsetof(class_name, member_name)))

//! Helper class for tracking floating point context and task group context
//! switches
/** Assuming presence of an itt collector, in addition to keeping track of
   floating point context, this class emits itt events to indicate begin and end
   of task group context execution **/
template <bool report_tasks> class context_guard_helper {
...
public:
  ...
  void set_ctx(const d1::task_group_context *ctx) {
    if (!ctx)
      return;
    const d1::cpu_ctl_env *ctl =
        reinterpret_cast<const d1::cpu_ctl_env *>(&ctx->my_cpu_ctl_env);
    // Compare the FPU settings directly because the context can be reused
    ...
    if (report_tasks && ctx != curr_ctx) {
      // if task group context was active, report end of current execution
      // frame.
      if (curr_ctx)
        ITT_TASK_END;
...
inline void prolonged_pause_impl() {
  // Assumption based on practice: 1000-2000 ticks seems to be a suitable
  // invariant for the majority of platforms. Currently, skip platforms that
  // define __TBB_STEALING_PAUSE because these platforms require very careful
  // tuning.
  ...
  do {
    ...
    if (curr <= prev)
      // Possibly, the current logical thread is moved to another hardware
      // thread or overflow is occurred.
      break;
    prev = curr;
  } while (prev < finish);
}
...
#if defined(__TPAUSE) && !defined(__amd64) && !defined(__AMD64)
    std::uint64_t time_stamp = machine_time_stamp();
    // _tpause function directs the processor to enter an
    // implementation-dependent optimized state until the Time Stamp Counter
    // reaches or exceeds the value specified in second parameter. Constant
    // "1000" is ticks to wait for.
    // TODO : Modify this parameter based on empirical study of benchmarks.
    // First parameter 0 selects between a lower power (cleared) or faster
    // wakeup (set) optimized state.
...
public:
  // my_yield_threshold = 100 is an experimental value. Ideally, once we start
  // calling __TBB_Yield(), the time spent spinning before calling out_of_work()
  // should be approximately the time it takes for a thread to be woken up.
  // Doing so would guarantee that we do no worse than 2x the optimal spin time.
  // Or perhaps a time-slice quantum is the right amount.
  stealing_loop_backoff(int num_workers, int yields_multiplier)
      : my_pause_threshold{2 * (num_workers + 1)}
#if __APPLE__
        // threshold value tuned separately for macOS due to high cost of
        // sched_yield there
        ,
  ...
  void reset_wait() { my_pause_count = my_yield_count = 0; }
};
...
/** Together with generic_scheduler::my_context_state_propagation_epoch forms
    cross-thread signaling mechanism that allows to avoid locking at the hot
   path of normal execution flow.

    When a descendant task group context is registered or unregistered, the
   global and local epochs are compared. If they differ, a state change is being
   propagated, and thus registration/deregistration routines take slower branch
   that may block (at most one thread of the pool can be blocked at any moment).
   Otherwise the control path is lock-free and fast. **/
...
#if TBB_USE_ASSERT
static const std::uintptr_t venom =
    tbb::detail::select_size_t_constant<0xDEADBEEFU,
                                        0xDDEEAADDDEADBEEFULL>::value;

inline void poison_value(std::uintptr_t &val) { val = venom; }

inline void poison_value(std::atomic<std::uintptr_t> &val) {
  val.store(venom, std::memory_order_relaxed);
}
...
inline void assert_task_valid(const d1::task *t) { assert_pointer_valid(t); }
#else /* !TBB_USE_ASSERT */
...
struct suspend_point_type {
  ...
  FastRandom m_random;
  //! The flag is raised when the original owner should return to this task
  //! dispatcher.
  ...
  void resume(suspend_point_type *sp) {
    __TBB_ASSERT(m_stack_state.load(std::memory_order_relaxed) !=
                     stack_state::suspended,
                 "The stack is expected to be active");
  ...
  void finilize_resume() {
    ...
    // Set the suspended state for the stack that we left. If the state is
    // already notified, it means that someone already tried to resume our
    // previous stack but failed. So, we need to resume it. m_prev_suspend_point
    // might be nullptr when destroying co_context based on threads
    if (m_prev_suspend_point &&
        m_prev_suspend_point->m_stack_state.exchange(stack_state::suspended) ==
            stack_state::notified) {
  ...
  bool try_notify_resume() {
    ...
    return m_stack_state.exchange(stack_state::notified) ==
           stack_state::suspended;
  }
...
public:
  // TODO: reconsider low level design to better organize dependencies and
  // files.
  ...
  /** Gets a task only if it has not been executed by its sender or a thief
      ...
      from its mailbox. (In contrast to local task pool, mailbox can be read
     only by its owner). **/
  d1::task *get_mailbox_task(mail_inbox &my_inbox, execution_data_ext &ed,
                             isolation_type isolation);

  d1::task *get_critical_task(d1::task *, execution_data_ext &, isolation_type,
                              bool);

  template <bool ITTPossible, typename Waiter>
  d1::task *receive_or_steal_task(thread_data &tls, execution_data_ext &ed,
                                  Waiter &waiter, isolation_type isolation,
                                  bool outermost, bool criticality_absence);
...
public:
  ...
  static void execute_and_wait(d1::task *t, d1::wait_context &wait_ctx,
  ...
  d1::task *get_inbox_or_critical_task(execution_data_ext &, mail_inbox &,
                                       isolation_type, bool);
  d1::task *get_stream_or_critical_task(execution_data_ext &, arena &,
                                        task_stream<front_accessor> &,
                                        unsigned & /*hint_for_stream*/,
                                        isolation_type,
                                        bool /*critical_allowed*/);
  d1::task *steal_or_get_critical(execution_data_ext &, arena &,
                                  unsigned /*arena_index*/, FastRandom &,
...
inline std::uintptr_t calculate_stealing_threshold(std::uintptr_t base,
...
struct task_group_context_impl {
  ...
  static void
  propagate_task_group_state(d1::task_group_context &,
                             std::atomic<uint32_t> d1::task_group_context::*,
                             d1::task_group_context &, uint32_t);
  ...
  static void copy_fp_settings(d1::task_group_context &ctx,
...
//! Forward declaration: throws std::runtime_error with what() returning
//! error_code description prefixed with aux_info
